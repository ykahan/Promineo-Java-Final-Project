...
class CustomerUtilImplTest {
	...
	void testCalculateBillTotal() {
		populateCustomers();
		assertEquals(28.04, util.calculateBillTotal(customers.get(0).getStatement().get(0)));
	}
	
	@Test
	void testCalculateCustomerTotal() {
		populateCustomers();
		assertEquals(576.04, util.calculateCustomerTotal(customers.get(1)));
	}
	
	@Test
	void testCalculatePastDue() {
		populateCustomers();
		assertEquals(28.04, util.calculatePastDue(customers.get(1)));
	}
	
	@Test
	void testCalculateAmountDueIn30Days() {
		populateCustomers();
		assertEquals(152.66, util.calculateAmountDueIn30Days(customers.get(1)));
	}
	
	@Test
	void testCalculateAmountPaid() {
		populateCustomers();
		assertEquals(423.38, util.calculateAmountPaid(customers.get(1)));
	}
	
	@Test
	void testCalculateCustomerWithMostOwed() {
		populateCustomers();
		assertEquals(customers.get(1), util.calculateCustomerWithMostOwed(customers));
	}
	
	@Test
	void testCalculateTotalPaidByState() {
		populateCustomers();
		assertEquals(697.38, util.calculateTotalPaidByState(customers, "AZ"));
	}
	
	@Test
	void testCalculateHighestPayingCustomer() {
		populateCustomers();
		assertEquals(customers.get(1), util.calculateHighestPayingCustomer(customers));
	}
	
	@Test
	void testCalculateStateWithGreatestReceivables() {
		populateCustomers();
